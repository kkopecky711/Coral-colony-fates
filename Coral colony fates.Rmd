...
plot18.whole_plots <- read_csv("Data/Plot 18/Plot18_2017-2020_unmatched.csv") %>% 
  clean_names() %>% 
...
plot18.summary.year <- plot18.whole_plots %>% 
  group_by(year) %>% 
...
plot18.summary.classes <- plot18.whole_plots %>% 
  group_by(year,tag_lab_class_name) %>% 
  summarize(total_surf = sum(tag_lab_surf_area),
            surf_m2 = total_surf*0.0001) %>% 
  select(-total_surf)

# Create combined dataframe and calculate propotional cover by each class
